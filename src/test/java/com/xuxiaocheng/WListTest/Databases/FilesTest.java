...
public class FilesTest {
    ...
    public void insertFilesSameDirectory() throws SQLException {
        FileManager.getInstance("test").insertFilesSameDirectory(List.of(
        ...
        Assertions.assertEquals(-1, Objects.requireNonNull(this.manager().selectFile(0, true, null)).size());
        FileManager.getInstance("test").insertFilesSameDirectory(List.of(
                new FileInformation(14, 11, "2-file", false, 234, null, null, null)
        ).iterator(), 11, null);
        Assertions.assertEquals(234, Objects.requireNonNull(this.manager().selectFile(11, true, null)).size());
        Assertions.assertEquals(-1, Objects.requireNonNull(this.manager().selectFile(0, true, null)).size());
        FileManager.getInstance("test").insertFilesSameDirectory(List.of(
                new FileInformation(15, 12, "2-file", false, 345, null, null, null)
        ).iterator(), 12, null);
        Assertions.assertEquals(345, Objects.requireNonNull(this.manager().selectFile(12, true, null)).size());
        Assertions.assertEquals(123 + 234 + 345, Objects.requireNonNull(this.manager().selectFile(0, true, null)).size());
    }
}
