...
use crate::handlers::bytes_util::{read_string, read_u8_vec, write_u8_array};
use crate::handlers::codec::{length_based_decode, length_based_encode};
...
impl WListClient {
    pub fn new(address: &String) -> Result<WListClient, io::Error> {
        ...
        let mut receiver = length_based_decode(&mut client)?;
        ...
        length_based_encode(client.get_mut(), &mut sender)?;
        Ok(WListClient { stream: client, key: GenericArray::clone_from_slice(key), vector: GenericArray::clone_from_slice(vector) })
    }

    pub fn send(client: &mut WListClient, message: &Vec<u8>) -> Result<(), io::Error> {
