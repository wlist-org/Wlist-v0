...
fn main() -> Result<(), io::Error> {
    ...
    let menu = PrintTable::create(from_slice(&vec!["ID", "Operation", "Permission", "Detail"]))
        ...
        .finish();
    let permissions_table: PrintTableCached = PrintTable::create(from_slice(&vec!["id", "policy", "level", "detail"]))
        .add_body(from_slice(&vec!["1", "ServerOperate", "DANGEROUS", "Operate server state."]))
        .add_body(from_slice(&vec!["2", "Broadcast", "WARNING", "Send broadcast to other connections."]))
        .add_body(from_slice(&vec!["3", "UsersList", "NORMAL", "Get users and user groups list."]))
        .add_body(from_slice(&vec!["4", "UsersOperate", "WARNING", "Modify users and user groups."]))
        .add_body(from_slice(&vec!["5", "DriverOperate", "DANGEROUS", "Operate web drivers."]))
        .add_body(from_slice(&vec!["6", "FilesBuildIndex", "WARNING", "Build indexs of web drivers."]))
        .add_body(from_slice(&vec!["7", "FilesList", "NORMAL", "Get files list."]))
        .add_body(from_slice(&vec!["8", "FileDownload", "NORMAL", "Download explicit file."]))
        .add_body(from_slice(&vec!["9", "FileUpload", "NORMAL", "Upload file to explicit path."]))
        .add_body(from_slice(&vec!["10", "FileDelete", "NORMAL", "Delete explicit file."]))
        .finish();
